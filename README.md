...
- 支援個人會員與商業公司註冊。
- 非 Google 帳號註冊不需填寫密碼，系統會寄出信箱驗證連結與初始登入資料。
- 公司會員初始登入帳號與密碼皆為統一編號；個人會員初始登入帳號與密碼皆為身分證字號（英文大寫）。
- 未完成信箱驗證者，狀態為「未驗證信箱」。
...
- 支援個人與公司會員使用 Google 帳號註冊流程，需於 `.env` 設定 Google OAuth 參數。
- 手機電話支援台灣手機號碼格式檢核。
- 市話支援台灣區域號碼欄位。

### 會員登入與會員中心

- 會員登入頁標題為「新零售行銷多元平台 - NewPay」，公司會員以統一編號與密碼登入，個人會員以身分證字號與密碼登入，並支援 Google 帳號登入入口。
- 登入唯一辨識以 Email 加身分證號或統一編號組合判斷，支援同 Email 同時註冊個人與公司法人。
- 公司法人登入需輸入統一編號；個人會員登入需輸入身分證號。
- 已完成信箱驗證但仍待後台審核的會員可登入會員中心，只能查看基本資料，並顯示「會員資料審核中」。
...
- 會員審核、停用、刪除。
- 會員資料可進入單獨編輯頁修改。
- 會員編輯頁可重新發送信箱驗證信與初始登入資料，提供未驗證會員重新驗證。
...
- 會員編號：`member_code`。
- 經銷商旗標：`is_dealer`。
- 信箱驗證：`email_verified_token`、`email_verified_at`。
