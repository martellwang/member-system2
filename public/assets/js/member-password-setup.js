...
document.getElementById('member-password-setup-form')?.addEventListener('submit', async (event) => {
  ...
  try {
    const res = await fetch(`${API}/members/setup-password/${encodeURIComponent(MEMBER_PASSWORD_SETUP_TOKEN)}`, {
      method: 'POST',
      headers: {
    ...
    window.setTimeout(() => {
      window.location.href = data.login_url || `${APP_BASE}/login`;
    }, 900);
  } catch (error) {
