// Taiwan county/city and district helpers for contact address fields.
(function () {
  const districts = {
  ...
  function bindTaiwanAddressSelects(cityId, districtId, selectedDistrict = '') {
    const city = document.getElementById(cityId);
    const district = document.getElementById(districtId);
    if (!city || !district) return;

    const selectedCity = city.dataset.selected || city.value;
    city.innerHTML = options(Object.keys(districts), '縣市');
    if (selectedCity && districts[selectedCity]) {
      city.value = selectedCity;
    }

    function renderDistricts() {
      const items = districts[city.value] || [];
      district.innerHTML = options(items, '地區');
      const currentDistrict = district.dataset.selected || selectedDistrict;
      if (currentDistrict && items.includes(currentDistrict)) {
  ...
  window.TAIWAN_DISTRICTS = districts;
  window.bindTaiwanAddressSelects = bindTaiwanAddressSelects;
}());
